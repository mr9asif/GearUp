...
const createGear = async (providerId, payload, files) => {
    ...
    if (files?.length) {
        for (const file of files) {
            const uploadedImage = await (0, uploadToCloudinary_1.uploadToCloudinary)(file, "gearup/gear");
            imageUrls.push(uploadedImage.secure_url);
        }
    }
    // Create gear
    const gear = await prisma_1.prisma.gearItem.create({
        data: {
            providerId,
...
const getAllGear = async (query) => {
    ...
    const total = await prisma_1.prisma.gearItem.count({
        where,
    });
    return {
        meta: {
...
// getmyowngear
const getMyGear = async (providerId, query) => {
    const page = Number(query.page) || 1;
    const limit = Number(query.limit) || 10;
    const skip = (page - 1) * limit;
    const search = query.search || "";
    ...
    const gears = await prisma_1.prisma.gearItem.findMany({
        where,
        skip,
        take: limit,
        orderBy: {
...
// getmysingle gear
const getMySingleGear = async (providerId, gearId) => {
    const gear = await prisma_1.prisma.gearItem.findUnique({
        where: {
            id: gearId,
        },
    ...
    if (gear.providerId !== providerId) {
        throw new Apperror_1.default(http_status_1.default.FORBIDDEN, "You are not authorized to access this gear.");
    }
    return gear;
};
// update gear
const updateGear = async (providerId, gearId, payload) => {
    ...
    if (gear.providerId !== providerId) {
        throw new Apperror_1.default(http_status_1.default.FORBIDDEN, "You are not authorized to update this gear.");
    }
    ...
    const updatedGear = await prisma_1.prisma.gearItem.update({
        where: {
            id: gearId,
        },
        data: {
...
const deleteGear = async (providerId, gearId) => {
    ...
    if (gear.providerId !== providerId) {
        throw new Apperror_1.default(http_status_1.default.FORBIDDEN, "You are not authorized to delete this gear.");
    }
    await prisma_1.prisma.gearItem.delete({
        where: {
            id: gearId,
        },
    });
...
const getAllGearService = async (query) => {
    const { search, category, brand, minPrice, maxPrice, available, page = "1", limit = "10", sortBy = "createdAt", sortOrder = "desc", } = query;
    const where = {};
